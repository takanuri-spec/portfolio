import { Service } from '../components/ServiceCard';

export const services: Service[] = [
  {
    id: '1',
    title: 'OH-SOUJI',
    description: '年末の大掃除をレトロゲーム風のクエストに変換',
    fullDescription: '年末の大掃除をレトロゲーム風のクエストに変換。ポイント制やボス戦演出で家族の競争心を引き出す。非エンジニアでもテキストファイルを書き換えるだけでタスク追加可能な簡易運用フローを実現。',
    tags: ['HTML', 'CSS', 'JavaScript', 'Firebase RTDB'],
    link: 'https://oosouji-quest.web.app/',
    year: '2025',
    imageUrl: '/assets/screenshots/スクリーンショット 2026-02-21 1.18.05.png',
  },
  {
    id: '2',
    title: 'TANA-OROSHI',
    description: 'カメラ撮影だけでAIが属性を自動解析',
    fullDescription: 'カメラ撮影だけでAIが属性を自動解析。断捨離アドバイスまで出力する未来のアシスタント。全資産を可視化し、執着を手放すための在庫管理基盤として設計。',
    tags: ['React', 'TypeScript', 'Gemini 2.0', 'Firestore'],
    link: 'https://tanaoroshi-810eb.web.app/',
    year: '2025',
    imageUrl: '/assets/screenshots/スクリーンショット 2026-02-21 1.20.20.png',
  },
  {
    id: '3',
    title: 'MONEY BOY',
    description: '家計簿から年金、RSU、確定申告まで',
    fullDescription: '家計簿から年金、RSU、確定申告まで。パーソナルな未来推計を実現する財務管理基盤。DBとしてGoogle Sheetsを採用し、自身が直接シート上でデータを視認・修正できるハック的構成。',
    tags: ['Vite PWA', 'NextAuth', 'Google Sheets API', 'Vercel'],
    link: 'https://money-boy.vercel.app/ledger/dashboard',
    year: '2025',
    imageUrl: '/assets/screenshots/スクリーンショット 2026-02-21 14.25.43.png',
  },
  {
    id: '4',
    title: 'TDTK',
    description: '地球一周を目指しGPSで歩行記録',
    fullDescription: '地球一周を目指しGPSで歩行記録。伊能忠敬の偉業というロマンと重ね合わせた継続ランニングアプリ。ただの距離記録を「歴史的な挑戦」に塗り替え、継続のモチベーションに変える。',
    tags: ['React', 'Vite', 'Capacitor', 'Geolocation API'],
    link: 'https://tracing-tadataka.web.app/',
    year: '2025',
    imageUrl: '/assets/screenshots/unnamed (5).png',
  },
  {
    id: '5',
    title: 'WORD BOKUJOU',
    description: '2つの言葉をAIで交配し新しい概念を生成',
    fullDescription: '2つの言葉をAIで交配し、予想外の新しい概念とイラストが誕生する創造的育成ゲーム。「文豪風」「中二病風」などの出力を操作できる『調味料システム』によるAI制御を実装。',
    tags: ['React', 'Vite', 'Gemini 2.0', 'Google Imagen'],
    link: 'https://wordbokujou.web.app/',
    year: '2026',
    imageUrl: '/assets/screenshots/スクリーンショット 2026-02-21 15.43.59.png',
  },
  {
    id: '6',
    title: '10 Days Ago',
    description: '10日前の市場データで投資判断を体験',
    fullDescription: '10日前の市場データだけで判断し、現在価格との差でスコアを競う投資シミュレーション。「10日前の自分ならどう判断したか？」を追体験し、投資の目利きを養うゲーム。',
    tags: ['React', 'Firestore', 'Functions', 'Financial API'],
    link: 'https://tendaysago-277e6.web.app/',
    year: '2026',
    imageUrl: '/assets/screenshots/スクリーンショット 2026-02-21 16.40.13.png',
  },
  {
    id: '7',
    title: 'PEPETETSU',
    description: '無限マップ生成ボードゲームエンジン',
    fullDescription: '無限マップ生成ボードゲームエンジン。プレイするたびに異なるマップが生成される、リプレイ性の高いボードゲーム体験を提供する。',
    tags: ['Firebase'],
    link: 'https://pepetetsu-de422.web.app/',
    year: '2026',
    imageUrl: '/assets/screenshots/pepetetsu.png',
  },
  {
    id: '8',
    title: 'MONEY-BOY NEO',
    description: 'Google Sheetsをデータストアとするパーソナル家計管理PWA',
    fullDescription: 'データの主権はユーザーに——すべての家計データは自分のGoogle Sheetsに保存。複式簿記・厚生年金シミュレーション・確定申告計算を一元管理するPWA。インストールするとホーム画面から起動でき、オフライン時もUIを表示。',
    tags: ['React 19', 'PWA', 'Google Sheets API', 'Vercel'],
    link: 'https://money-boy-8cuw.vercel.app/',
    year: '2026',
    imageUrl: '/assets/screenshots/スクリーンショット 2026-02-21 14.25.43.png',
  },
  {
    id: '9',
    title: 'ONTAKE100 2026',
    description: '100マイルの過酷なトレイルランニングに挑むランナー向け非公式ガイド',
    fullDescription: '日本屈指の本格派トレイルランニングレース「ONTAKE100」の非公式ガイド。コース概要、エイド到着予想から必携品リストまで、当日の完走を後押しする情報をPWAに集約。過酷な疲労下でも直感的に情報へアクセスできるよう、黒ベースの視認性の高いUIを採用。',
    tags: ['React 19', 'TypeScript', 'Vite', 'PWA'],
    link: 'https://ontake100-2026.web.app',
    year: '2026',
    imageUrl: '/assets/screenshots/ontake100.png',
  },
  {
    id: '10',
    title: '白地図 (Shirochizu)',
    description: 'GPSで市区町村の訪問状況を塗りつぶす自分だけの白地図アプリ',
    fullDescription: '訪れたことのある場所に色を塗り、自分だけの訪問マップを作成・記録できるWebアプリケーション（PWA）。現在は東京都に対応。D3.jsとTopoJSONを活用した高精度なSVG地図描画と、実際に現地でGPS認証を行う「訪問証明」機能を搭載している。',
    tags: ['React 19', 'TypeScript', 'D3.js', 'Firebase Auth', 'Firestore'],
    link: 'https://shirochizuumetai.web.app',
    year: '2026',
    imageUrl: '/assets/screenshots/shirochizu_app.png',
  },
  {
    id: '11',
    title: 'AIR BLDG 3D',
    description: 'Godotで再現したオフィスビルの3Dウォークスルーマップ',
    fullDescription: 'GodotエンジンとGDScriptで構築したリアルなオフィスビル3Dマップ。FPS視点でフロアを自由に歩き回れる。照明の昼夜切り替え、ミニマップ表示、図面全体表示など探索を支援する機能を搭載。WebAssemblyにコンパイルしてブラウザ上で動作させている。',
    tags: ['Godot', 'GDScript', 'WebAssembly', 'Firebase Hosting'],
    link: 'https://toaru-office-building.web.app/',
    year: '2026',
    imageUrl: '/assets/screenshots/toaru_office_building.png',
  },
  {
    id: '12',
    title: '伊勢志摩 旅のしおり',
    description: '三重伊勢志摩2026家族旅行の旅程をまとめたデジタルしおり',
    fullDescription: '2026年夏の三重・伊勢志摩家族旅行（8月5日〜7日）のためにつくったデジタルしおりアプリ。Googleログインで家族だけが閲覧できる認証付きプライベートWebアプリ。旅程・観光スポット・宿泊情報などをスマートフォンで手軽に確認できるよう設計。',
    tags: ['React', 'Firebase Auth', 'Firebase Hosting'],
    link: 'https://summer-iseshima.web.app/',
    year: '2026',
    imageUrl: '/assets/screenshots/summer_iseshima.png',
  },
];
